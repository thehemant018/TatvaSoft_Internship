...
export class MissionApplicationComponent implements OnInit {
  ...
  ApproveMissionApplication(missionApplicationId: number) {
    // console.log(missionApplicationId);
    this.service.MissionApplicationApprove(missionApplicationId).subscribe(
      (data: any) => {
        if (data.result == 1) {
          this.toast.success({ detail: "SUCCESS", summary: data.data, duration: 3000 });
          setTimeout(() => {
  ...
  DeleteMissionApplication(missionApplicationId: number) {
    this.service.MissionApplicationDelete(missionApplicationId).subscribe(
      (data: any) => {
        if (data.result == 1) {
          this.toast.success({ detail: "SUCCESS", summary: data.data, duration: 3000 });
          setTimeout(() => {
